...
function downloadExtractRegisterMumble() {
  request
  .get('https://github.com/SpawnHouse/MumblePortable/releases/latest/download/MumblePortable.zip')
...
ipcMain.on("downloadExtractRegisterMumbleRequest", (event, args) => {
  ...
  mainWindow.webContents.send("downloadExtractRegisterMumbleResponse", {'status':'ok'});
});

ipcMain.on("downloadExtractMinecraftLatestRequest", (event, args) => {
  request
  .get('https://github.com/SpawnHouse/Minecraft/releases/latest/download/Minecraft.zip')
  .on('error', function(error) {
    console.log(error);
  })
  ...
  .on('finish', function() {
    ...
    fs.copyFile("./Minecraft/mumblelink-1.16.5-4.6.3.jar", process.env.APPDATA+"/.minecraft/mods/mumblelink-1.16.5-4.6.3.jar", (err) => {
      if (err) {
        console.log("Error Found:", err);
      }
    });
    fs.copyFile("./Minecraft/tlauncher-2.0.properties", process.env.APPDATA+"/.tlauncher/tlauncher-2.0.properties", (err) => {
      if (err) {
        console.log("Error Found:", err);
      }
    });
  ...
  mainWindow.webContents.send("downloadExtractMinecraftLatestResponse", {'status':'ok'});
});
...
ipcMain.on("launchMinecraftMumbleRequest", (event, args) => {
  ...
  require('child_process').exec(process.cwd()+'/MumblePortable/mumble.exe mumble://mumble.spawnhouse.com/Minecraft', 
    function(err) {
      console.log(err);
    });
  mainWindow.webContents.send("launchMinecraftMumbleResponse", {'status':'ok'});
});